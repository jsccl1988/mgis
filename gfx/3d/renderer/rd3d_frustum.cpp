...
// get frustum matrix
void Frustum::GetFrustum(float frustum[6][4]) {
  memcpy(frustum, m_frustum, 24 * sizeof(float));
}

// set frustum matrix
void Frustum::SetFrustum(float frustum[6][4]) {
  memcpy(m_frustum, frustum, 24 * sizeof(float));
}

bool Frustum::IsPointIn(float x, float y, float z) {
  // Go through all the sides of the frustum
  for (int i = 0; i < 6; i++) {
    // Calculate the plane equation and check if the point is behind a side of
    // the frustum
    if (m_frustum[i][P_A] * x + m_frustum[i][P_B] * y + m_frustum[i][P_C] * z +
            m_frustum[i][P_D] <=
        0) {
...
bool Frustum::IsSphereIn(float x, float y, float z, float radius) {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if (m_frustum[i][P_A] * x + m_frustum[i][P_B] * y + m_frustum[i][P_C] * z +
            m_frustum[i][P_D] <=
        -radius) {
      // The distance was greater than the radius so the sphere is outside of
      // the frustum
      return false;
    }
  }
...
bool Frustum::IsCuboidIn(float x, float y, float z, SIZE size) {
  for (int i = 0; i < 6; i++) {
    if (m_frustum[i][P_A] * (x - size.cx) + m_frustum[i][P_B] * (y - size.cy) +
...
bool Frustum::IsBoxIn(float max_x, float max_y, float max_z, float min_x,
