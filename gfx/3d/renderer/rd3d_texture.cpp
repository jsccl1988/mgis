...
namespace _3Drd {
Texture::Texture(LP3DRENDERDEVICE p3DRenderDevice, uint handle, string strName)
    : m_unHandle(handle),
...
void Texture::SetSampler(TextureSampler sampler) { m_texSampler = sampler; }

TextureEnvMode Texture::GetEnvMode() const { return m_texEnv; }

void Texture::SetEnvMode(TextureEnvMode envMode) { m_texEnv = envMode; }

TextureDesc Texture::GetDesc() const { return m_texDesc; }

int Texture::GetComponents(TextureFormat format) {
  switch (format) {
    case ALPHA16F:
    case ALPHA32F:
    case INTENSITY8:
...
bool Texture::IsSizePowerOfTwo() {
  return (((m_texDesc.width & (m_texDesc.width - 1)) == 0) &&
          ((m_texDesc.height & (m_texDesc.height - 1)) == 0));
}

long Texture::Create(ulong ulWidth, ulong ulHeight, TextureFormat format,
                     bool bDynamic, bool bUseMips) {
