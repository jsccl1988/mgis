...
CombinedCamera::~CombinedCamera(void) {}

void CombinedCamera::SetCamera(Vector3 &pos, Vector3 &view, Vector3 &up) {
  m_vEye = pos;
  m_vUp = up;
  m_vTarget = view;
}
...
// move camera
void CombinedCamera::ShiftCamera(float step) {
...
void CombinedCamera::ForwardCamera(float step) {
...
void CombinedCamera::RiseCamera(float step) {
  m_vEye.y += m_vUp.y * step;
  m_vTarget.y += m_vUp.y * step;
}

void CombinedCamera::LeanCamera(float angle) {
  Vector3 vDir = m_vTarget - m_vEye;
  vDir.Normalize();
  m_vUp.Rotate(vDir, angle);
}

void CombinedCamera::MoveCamera(Vector3 &vec) {
  m_vEye += vec;
  m_vTarget += vec;
}

void CombinedCamera::MoveCameraToPos(Vector3 &pos) {
  Vector3 dV = pos - m_vEye;
  MoveCamera(dV);
}

void CombinedCamera::RaiseViewDirection(float angle) {
  Vector3 vCross, vDir(m_vTarget - m_vEye);
  vCross = vDir.CrossProduct(m_vUp);
  vCross.Normalize();
...
void CombinedCamera::TurnViewDirection(float angle) {
  Vector3 vDir(m_vTarget - m_vEye);
...
//////////////////////////////////////////////////////////////////////////
void CombinedCamera::SetSphereCameraMove(long deltX, long deltY) {
  // 从聚焦点指向视点的向量  OA向量
