...
long GLRenderDevice::Transform2DTo3D(Vector3 &vOrg, Vector3 &vTar,
                                     const lPoint &point) {
...
// 3d to 2d
long GLRenderDevice::Transform3DTo2D(const Vector3 &ver3D, lPoint &point) {
  GLdouble _x = 0., _y = 0., _z = 0.;
  GLdouble modelview[16];
  GLdouble projection[16];
  GLint viewport[4];
...
// get frustum
long GLRenderDevice::GetFrustum(Frustum &smtFrustum) {
  float frustum[6][4];
