...
namespace _3Drd {
FBOFuncImpl::FBOFuncImpl() {}

FBOFuncImpl::~FBOFuncImpl() {}

long FBOFuncImpl::Initialize(LPGLRENDERDEVICE pGLRenderDevice) {
  _glGenFramebuffers =
  ...
  _glIsFramebuffer = (PFNGLISFRAMEBUFFEREXTPROC)pGLRenderDevice->GetProcAddress(
      "glIsFramebufferEXT");
  _glGenRenderbuffers =
  ...
  if (NULL == _glGenFramebuffers || NULL == _glDeleteFramebuffers ||
      NULL == _glBindFramebuffer || NULL == _glIsFramebuffer ||
      NULL == _glGenRenderbuffers || NULL == _glDeleteRenderbuffers ||
      NULL == _glBindRenderbuffer || NULL == _glIsRenderbuffer ||
...
void FBOFuncImpl::glDeleteFramebuffers(GLsizei count, GLuint *ids) {
  _glDeleteFramebuffers(count, ids);
}

void FBOFuncImpl::glBindFramebuffer(GLenum target, GLuint id) {
  _glBindFramebuffer(target, id);
}

GLboolean FBOFuncImpl::glIsFramebuffer(GLuint id) {
  return _glIsFramebuffer(id);
}

void FBOFuncImpl::glGenRenderbuffers(GLsizei count, GLuint *ids) {
  _glGenRenderbuffers(count, ids);
}

void FBOFuncImpl::glDeleteRenderbuffers(GLsizei count, GLuint *ids) {
  _glDeleteRenderbuffers(count, ids);
}

void FBOFuncImpl::glBindRenderbuffer(GLenum target, GLuint id) {
  _glBindRenderbuffer(target, id);
}

GLboolean FBOFuncImpl::glIsRenderbuffer(GLuint id) {
  return _glIsRenderbuffer(id);
}

void FBOFuncImpl::glRenderbufferStorage(GLenum target, GLenum internalFormat,
                                        GLsizei width, GLsizei height) {
  _glRenderbufferStorage(target, internalFormat, width, height);
}

void FBOFuncImpl::glFramebufferRenderbuffer(GLenum target, GLenum attachment,
                                            GLenum rbTarget, GLuint rbId) {
  _glFramebufferRenderbuffer(target, attachment, rbTarget, rbId);
}

int FBOFuncImpl::getMaxColorAttachments() {
  int maxColors = 0;
  glGetIntegerv(GL_MAX_COLOR_ATTACHMENTS_EXT, &maxColors);
  return maxColors;
}

void FBOFuncImpl::glFramebufferTexture1D(GLenum target, GLenum attachment,
                                         GLenum texTarget, GLuint texId,
                                         int level) {
  _glFramebufferTexture1D(target, attachment, texTarget, texId, level);
}

void FBOFuncImpl::glFramebufferTexture2D(GLenum target, GLenum attachment,
                                         GLenum texTarget, GLuint texId,
                                         int level) {
  _glFramebufferTexture2D(target, attachment, texTarget, texId, level);
}

void FBOFuncImpl::glFramebufferTexture3D(GLenum target, GLenum attachment,
                                         GLenum texTarget, GLuint texId,
                                         int level, int zOffset) {
  _glFramebufferTexture3D(target, attachment, texTarget, texId, level, zOffset);
}

GLenum FBOFuncImpl::glCheckFramebufferStatus(GLenum target) {
