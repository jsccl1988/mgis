...
enum eSelectMode { ST_Point, ST_Rect, ST_Circle, ST_Polygon };
...
enum ePointType { PT_DOT, PT_ChildImage, PT_Text };

enum eRegionType { RT_Fan, RT_Rect, RT_Polygon };

enum IAType { eIA_AppendFeature, eIA_DeleteFeature, eIA_ModifyFeature };
...
#define GT_MSG_GET_INPUT_POINT_TYPE \
  (GT_MSG_BEGIN_INPUT_POINT + 2)  //获取输入点类型
#define GT_MSG_GET_INPUT_ANNO_ANGLE \
  (GT_MSG_BEGIN_INPUT_POINT + 3)  //获取输入注记和角度(如果是输入注记的话)
#define GT_MSG_RET_INPUT_POINT (GT_MSG_BEGIN_INPUT_POINT + 4)  //返回输入点
#define GT_MSG_END_INPUT_POINT \
  (GT_MSG_BEGIN_INPUT_POINT + GT_TOOL_MAX_MSG_COUNT)  //点输入工具结束消息
...
#define GT_MSG_END_INPUT_REGION \
  (GT_MSG_BEGIN_INPUT_REGION + GT_TOOL_MAX_MSG_COUNT)
...
#define GT_MSG_CMD_VIEWCTRL_END \
  (GT_MSG_CMD_VIEWCTRL_BEGIN + GT_TOOL_MAX_MSG_COUNT)
...
#define GT_MSG_CMD_WSVIEWCTRL_END \
  (GT_MSG_CMD_WSVIEWCTRL_BEGIN + GT_TOOL_MAX_MSG_COUNT)
...
// point feature
#define GT_MSG_APPEND_POINT_CHILDIMAGE_FEATURE \
  (GT_MSG_CMD_APPEND_FEATURE_BEGIN + 1)
...
// line feature
#define GT_MSG_APPEND_LINE_SPLINE_LAG_FEATURE \
  (GT_MSG_CMD_APPEND_FEATURE_BEGIN + 4)
#define GT_MSG_APPEND_LINE_SPLINE_BZER_FEATURE \
  (GT_MSG_CMD_APPEND_FEATURE_BEGIN + 5)
#define GT_MSG_APPEND_LINE_SPLINE_B_FEATURE \
  (GT_MSG_CMD_APPEND_FEATURE_BEGIN + 6)
#define GT_MSG_APPEND_LINE_SPLINE_3_FEATURE \
  (GT_MSG_CMD_APPEND_FEATURE_BEGIN + 7)
#define GT_MSG_APPEND_LINE_LINESTRING_FEATURE \
  (GT_MSG_CMD_APPEND_FEATURE_BEGIN + 8)
...
#define GT_MSG_APPEND_LINE_LINEARRING_FEATURE \
  (GT_MSG_CMD_APPEND_FEATURE_BEGIN + 11)
...
#define GT_MSG_APPEND_SURF_POLYGON_FEATURE \
  (GT_MSG_CMD_APPEND_FEATURE_BEGIN + 14)
#define GT_MSG_CMD_APPEND_FEATURE_END \
  (GT_MSG_CMD_APPEND_FEATURE_BEGIN + GT_TOOL_MAX_CMD_COUNT)
...
#define GT_MSG_CMD_3DVIEWCTRL_END \
  (GT_MSG_CMD_3DVIEWCTRL_BEGIN + GT_TOOL_MAX_CMD_COUNT)
