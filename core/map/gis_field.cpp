...
void InitFieldNameToTypeMap(map<string, ushort> &fldNameToType) {
  ...
  fldNameToType.insert(
      map<string, ushort>::value_type("IntegerList", SmtIntegerList));
  fldNameToType.insert(
      map<string, ushort>::value_type("RealList", SmtRealList));
  fldNameToType.insert(
      map<string, ushort>::value_type("StringList", SmtStringList));
  ...
  fldNameToType.insert(
      map<string, ushort>::value_type("DateTime", SmtDateTime));
}
...
void InitFieldTypeToNameMap(map<ushort, string> &fldTypeToName) {
  ...
  fldTypeToName.insert(
      map<ushort, string>::value_type(SmtIntegerList, "IntegerList"));
  fldTypeToName.insert(
      map<ushort, string>::value_type(SmtRealList, "RealList"));
  fldTypeToName.insert(
      map<ushort, string>::value_type(SmtStringList, "StringList"));
  ...
  fldTypeToName.insert(
      map<ushort, string>::value_type(SmtDateTime, "DateTime"));
}
...
const double *SmtField::GetValueAsDoubleList(int *pnCount) const {
  ...
  if (m_fldValue.Vt == SmtRealList) {
    if (pnCount != NULL) *pnCount = m_fldValue.dbfValList.nCount;
...
byte *SmtField::GetValueAsBinary(int *pnCount) const {
  ...
  if (m_fldValue.Vt == SmtBinary) {
    if (pnCount != NULL) *pnCount = m_fldValue.blobVal.nCount;
...
int SmtField::GetValueAsDateTime(int *pnYear, int *pnMonth, int *pnDay,
                                 int *pnHour, int *pnMinute, int *pnSecond,
                                 int *pnTZFlag) const {
  if (!IsFieldSetted()) return ERR_FAILURE;

  if (m_fldValue.Vt == SmtDate || m_fldValue.Vt == SmtTime ||
      m_fldValue.Vt == SmtDateTime)

  {
    if (pnYear) *pnYear = m_fldValue.dateVal.Year;
    if (pnMonth) *pnMonth = m_fldValue.dateVal.Month;
    if (pnDay) *pnDay = m_fldValue.dateVal.Day;
    if (pnHour) *pnHour = m_fldValue.dateVal.Hour;
    if (pnMinute) *pnMinute = m_fldValue.dateVal.Minute;
    if (pnSecond) *pnSecond = m_fldValue.dateVal.Second;
    if (pnTZFlag) *pnTZFlag = m_fldValue.dateVal.TZFlag;
  ...
  switch (m_fldValue.Vt) {
    ...
    case SmtIntegerList:
      if (m_fldValue.iValList.paList != NULL) {
        SAFE_DELETE_A(m_fldValue.iValList.paList);
    ...
    case SmtRealList:
      if (m_fldValue.dbfValList.paList != NULL) {
        SAFE_DELETE_A(m_fldValue.dbfValList.paList);
    ...
    case SmtStringList: {
      ...
      SAFE_DELETE_A(m_fldValue.bstrValList.paList);

    } break;

    case SmtBinary:
      if (m_fldValue.blobVal.paData != NULL) {
        SAFE_DELETE_A(m_fldValue.blobVal.paData);
...
//////////////////////////////////////////////////////////////////////////
void SmtField::SetName(const char *name) {
  sprintf_s(m_fldName, FIELD_NAME_LENGTH, "%s", name);
}
...
int SmtField::SetValue(const SmtVariant &smtFld) {
  if (m_fldValue.Vt != smtFld.Vt) return ERR_FAILURE;
  ...
  switch (m_fldValue.Vt) {
    ...
    case SmtString:
      if (NULL == smtFld.bstrVal) return ERR_FAILURE;

      m_fldValue.bstrVal = strdup(smtFld.bstrVal);
      break;

    case SmtIntegerList: {
      if (NULL == smtFld.iValList.paList) return ERR_FAILURE;

      m_fldValue.iValList.paList = new int[smtFld.iValList.nCount];
      memcpy(m_fldValue.iValList.paList, smtFld.iValList.paList,
             sizeof(int) * smtFld.iValList.nCount);
      m_fldValue.iValList.nCount = smtFld.iValList.nCount;
    } break;

    case SmtRealList: {
      if (NULL == smtFld.dbfValList.paList) return ERR_FAILURE;

      m_fldValue.dbfValList.paList = new double[smtFld.dbfValList.nCount];
      memcpy(m_fldValue.dbfValList.paList, smtFld.dbfValList.paList,
             sizeof(double) * smtFld.dbfValList.nCount);
      m_fldValue.dbfValList.nCount = smtFld.dbfValList.nCount;
    } break;

    case SmtStringList: {
      if (NULL == smtFld.bstrValList.paList) return ERR_FAILURE;
    ...
    case SmtBinary: {
      if (NULL == smtFld.blobVal.paData) return ERR_FAILURE;

      m_fldValue.blobVal.paData = new byte[smtFld.blobVal.nCount];
      memcpy(m_fldValue.blobVal.paData, smtFld.blobVal.paData,
             sizeof(byte) * smtFld.blobVal.nCount);
...
int SmtField::SetValue(int nYear, int nMonth, int nDay, int nHour, int nMinute,
