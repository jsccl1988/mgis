...
SmtAttribute *SmtAttribute::Clone(void) const {
  ...
  if (NULL == pNewSmtAtt) return NULL;

  if (m_lFieldCount < 1) return pNewSmtAtt;
...
void SmtAttribute::AddField(SmtField &fld) {
  if (IsFieldExsit(fld.GetName())) return;

  m_pFieldPtrs = (SmtField **)realloc(m_pFieldPtrs,
                                      sizeof(SmtField *) * (m_lFieldCount + 1));
...
void SmtAttribute::RemoveField(const char *fldname) {
  ...
  if (index != -1 && index < m_lFieldCount) {
    SAFE_DELETE(m_pFieldPtrs[index]);
    memmove(m_pFieldPtrs + index, m_pFieldPtrs + index + 1,
            sizeof(void *) * (m_lFieldCount - index - 1));
    m_lFieldCount--;
  }
}
...
bool SmtAttribute::IsFieldExsit(const char *fldname) const {
  ...
  bool flag = false;
  while (!flag && i < m_lFieldCount) {
    if (strcmp(m_pFieldPtrs[i]->GetName(), fldname) == 0) flag = true;
    i++;
  }
  return flag;
}
