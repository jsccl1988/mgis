...
class EXPORT_CLASS SmtField {
 ...
 public:
  ...
  int GetValueAsDateTime(int *pnYear, int *pnMonth, int *pnDay, int *pnHour,
                         int *pnMinute, int *pnSecond, int *pnTZFlag) const;
  ...
  int SetValue(int nYear, int nMonth, int nDay, int nHour = 0, int nMinute = 0,
 ...
 protected:
  char m_fldName[FIELD_NAME_LENGTH];
  SmtVariant m_fldValue;
};

class EXPORT_CLASS SmtAttribute {
 public:
  SmtAttribute(void);
