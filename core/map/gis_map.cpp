  ...
  if (GetLayer(pLayer->GetLayerName()) == NULL) {
    if (m_vSmtLayers.size() == 0)
      m_MapEnvelope.MaxX = m_MapEnvelope.MinX = m_MapEnvelope.MaxY =
          m_MapEnvelope.MinY = C_INVALID_DBF_VALUE;
...
bool SmtMap::DeleteLayer(const char *name) {
  bool bRet = true;
  vector<SmtLayer *>::iterator iter = m_vSmtLayers.begin();
  while (iter != m_vSmtLayers.end()) {
    if (strcmp((*iter)->GetLayerName(), name) == 0) {
      if ((*iter) == m_pCurrentLayer) m_pCurrentLayer = NULL;
...
const SmtLayer *SmtMap::GetLayer(const char *name) const {
  SmtLayer *pLayer = NULL;
...
//////////////////////////////////////////////////////////////////////////
void SmtMap::MoveFirst(void) const { m_nIteratorIndex = 0; }

void SmtMap::MoveNext(void) const {
  if (m_nIteratorIndex < m_vSmtLayers.size()) m_nIteratorIndex++;
}
...
SmtLayer *SmtMap::GetLayer() { return GetLayer(m_nIteratorIndex); }

const SmtLayer *SmtMap::GetLayer() const { return GetLayer(m_nIteratorIndex); }

SmtLayer *SmtMap::GetLayer(int index) {
  if (index < 0 || index > (m_vSmtLayers.size() - 1)) return NULL;
...
bool SmtMap::AppendFeature(const SmtFeature *pC_Feature, bool bIsClone) {
  if (NULL == pC_Feature) return false;
  ...
  if (m_pCurrentLayer) {
    if (LYR_VECTOR == m_pCurrentLayer->GetLayerType()) {
      ...
      if (pFeature->GetFeatureType() == pVLayer->GetLayerFeatureType()) {
        bRet = (ERR_NONE == pVLayer->AppendFeature(pFeature, bIsClone));
      } else
...
bool SmtMap::DeleteFeature(const SmtFeature *pFeature) {
  ...
  if (m_pCurrentLayer) {
    if (LYR_VECTOR == m_pCurrentLayer->GetLayerType()) {
      ...
      if (pFeature->GetFeatureType() == pVLayer->GetLayerFeatureType()) {
        bRet = (ERR_NONE == pVLayer->DeleteFeature(pFeature));
      } else
        MessageBeep(0xFFFFFFFF);
    } else
      MessageBeep(0xFFFFFFFF);
...
bool SmtMap::UpdateFeature(const SmtFeature *pFeature) {
  ...
  if (m_pCurrentLayer) {
    if (LYR_VECTOR == m_pCurrentLayer->GetLayerType()) {
      ...
      if (pFeature->GetFeatureType() == pVLayer->GetLayerFeatureType()) {
        bRet = (ERR_NONE == pVLayer->UpdateFeature(pFeature));
      } else
        MessageBeep(0xFFFFFFFF);
    } else
      MessageBeep(0xFFFFFFFF);
...
bool SmtMap::QueryFeature(const SmtGQueryDesc *pC_GQueryDesc,
                          const SmtPQueryDesc *pC_PQueryDesc,
  ...
  if (m_pCurrentLayer) {
    if (LYR_VECTOR == m_pCurrentLayer->GetLayerType()) {
      ...
      pGQueryDesc->sSRs = (SmtSpatialRs)GetQueryRs(
          pGQueryDesc->pQueryGeom->GetGeometryType(), nFeaType);
      bRet = (pVLayer->Query(pGQueryDesc, pPQueryDesc, pQueryResult) ==
              ERR_NONE);
    }
  }
