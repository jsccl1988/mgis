...
long CopyLayer(SmtLayer *pTarLayer, SmtLayer *pSrcLayer, bool bClone,
  ...
  if (pSrcLayer->GetLayerType() == LYR_VECTOR)
    return CopyLayer((SmtVectorLayer *)pTarLayer, (SmtVectorLayer *)pSrcLayer,
                     bClone, bCheckFeaType);
  else if (pSrcLayer->GetLayerType() == LYR_RASTER)
    return CopyLayer((SmtRasterLayer *)pTarLayer, (SmtRasterLayer *)pSrcLayer,
                     bClone, bCheckFeaType);

  return ERR_FAILURE;
}

long CopyLayer(SmtVectorLayer *pTarLayer, SmtVectorLayer *pSrcLayer,
               bool bClone, bool bCheckFeaType) {
  if (!pTarLayer || !pSrcLayer) return ERR_INVALID_PARAM;
...
long CopyLayer(SmtRasterLayer *pTarLayer, SmtRasterLayer *pSrcLayer,
  ...
  if (ERR_NONE == pSrcLayer->GetRasterNoClone(pRasterBuf, lRasterBufSize,
                                                  locRect, lCodeType) &&
      ERR_NONE == pTarLayer->CreaterRaster(pRasterBuf, lRasterBufSize,
                                               locRect, lCodeType)) {
    pTarLayer->CalEnvelope();
  }
...
long Points2MultiPoint(SmtVectorLayer *pLayer, string strStyle) {
  ...
  if (ERR_NONE != pLayer->AppendFeature(pSmtFeature, false)) {
    SAFE_DELETE(pSmtFeature)
    return ERR_FAILURE;
  }
