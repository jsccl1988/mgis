...
void SmtFeature::SetFeatureType(SmtFeatureType type) {
  if (m_SmtFeatureType == type) return;
...
SmtFeature *SmtFeature::Clone() const {
  ...
  if (pNewFeature == NULL) return NULL;

  pNewFeature->SetID(GetID());
  pNewFeature->SetFeatureType(m_SmtFeatureType);
  pNewFeature->SetGeometry(m_pGeom);
  if (NULL != m_pAtt) pNewFeature->m_pAtt = m_pAtt->Clone();
  if (NULL != m_pStyle) pNewFeature->SetStyle(m_pStyle);

  return pNewFeature;
}

bool SmtFeature::Equal(const SmtFeature *poFeature) {
  if (poFeature == this) return true;

  if (GetID() != poFeature->GetID()) return false;
...
void SmtFeature::AddField(SmtField &fld) {
  if (m_pAtt) m_pAtt->AddField(fld);
}

void SmtFeature::RemoveField(const char *szFldName) {
  if (m_pAtt == NULL) return;
  m_pAtt->RemoveField(szFldName);
}

int SmtFeature::GetFieldIndexByName(const char *name) {
  if (m_pAtt == NULL) return -1;

  return m_pAtt->GetFieldIndex(name);
}

void SmtFeature::SetName(int index, const char *name) {
  if (m_pAtt == NULL) return;
...
int SmtFeature::SetFieldValue(int index, const SmtVariant &smtFld) {
  if (m_pAtt == NULL) return ERR_FAILURE;

  SmtField *pField = m_pAtt->GetFieldPtr(index);
  if (NULL == pField) return ERR_FAILURE;
...
int SmtFeature::SetFieldValue(int index, int nYear, int nMonth, int nDay,
  ...
  return pField->SetValue(nYear, nMonth, nDay, nHour, nMinute, nSecond,
                          nTZFlag);
}
...
int SmtFeature::SetStyle(const char *szStyleName) {
  ...
  if (pStyleMgr) {
    ...
    if (pStyle != NULL) SetStyle(pStyle);
  }
...
int SmtFeature::SetStyleDirectly(SmtStyle *pStyle) {
  if (pStyle == NULL) return ERR_FAILURE;
  SAFE_DELETE(m_pStyle);
  m_pStyle = pStyle;
  return ERR_NONE;
}
