#ifndef SDB_OGR_GDAL_H
#define SDB_OGR_GDAL_H
 ...
 */
class CORE_EXPORT gdal_error : public std::runtime_error {
  std::string m_driver;
 ...
 public:
  gdal_error(const std::string& message, OGRErr error,
             const std::string& driver = "", const std::string& dataset = "",
  ...
  const std::string& driver() const noexcept { return m_driver; }
  const std::string& dataset() const noexcept { return m_dataset; }
  const std::string& layer() const noexcept { return m_layer; }
  const std::string& field() const noexcept { return m_field; }
  OGRErr error() const noexcept { return m_error; }
...
#else
  init_wrapper() noexcept { OGRRegisterAll(); }
  ~init_wrapper() noexcept { OGRCleanupAll(); }
#endif
};  // struct init_wrapper

struct init_library {
  init_library() { static init_wrapper iw; }
  ...
  Driver(const std::string& driver_name)
      : init_library(),
        ...
        m_driver(OGRSFDriverRegistrar::GetRegistrar()->GetDriverByName(
            ...
            if (!m_driver) {
              throw gdal_error{
                  std::string{"unknown driver: '"} + driver_name + "'",
                  OGRERR_NONE, driver_name};
}
}  // namespace detail

gdal_driver_type& get() const noexcept { return *m_driver; }
};  // namespace core
...
struct Options {
  ...
  Options(const std::vector<std::string>& options)
      : m_options(options), m_ptrs(new const char*[options.size() + 1]) {
 ...
 public:
  SRS() : m_spatial_reference() {
    ...
    if (result != OGRERR_NONE) {
      throw gdal_error{
          std::string{"can not initialize spatial reference system WGS84"},
          result};
    }
  }
  ...
  explicit SRS(int epsg) : m_spatial_reference() {
    ...
    if (result != OGRERR_NONE) {
      throw gdal_error{
          std::string{"can not initialize spatial reference system for EPSG:"} +
              std::to_string(epsg),
          result};
    }
  }
  ...
  explicit SRS(const char* name) : m_spatial_reference() {
    ...
    if (result != OGRERR_NONE) {
      throw gdal_error{
          std::string{"can not initialize spatial reference system '"} + name +
              "'",
          result};
    }
  }
  ...
  OGRSpatialReference& get() noexcept { return m_spatial_reference; }
...
class Dataset {
 ...
 public:
  Dataset(const std::string& driver_name, const std::string& dataset_name,
          const SRS& srs = SRS{}, const std::vector<std::string>& options = {})
      : m_driver_name(driver_name),
        ...
        m_dataset(
            detail::Driver(driver_name)
                .get()
                .CreateDataSource(dataset_name.c_str(), m_options.get())) {
#endif
            if (!m_dataset) {
              throw gdal_error{std::string{"failed to create dataset '"} +
                                   dataset_name + "'",
                               OGRERR_NONE, driver_name, dataset_name};
}
}
...
const std::string& driver_name() const noexcept { return m_driver_name; }
const std::string& dataset_name() const noexcept { return m_dataset_name; }

gdal_dataset_type& get() const noexcept { return *m_dataset; }
SRS& srs() noexcept { return m_srs; }
...
class Layer {
 ...
 public:
  Layer(Dataset& dataset, const std::string& layer_name,
        OGRwkbGeometryType type, const std::vector<std::string>& options = {})
      : m_options(options),
        m_dataset(dataset),
        m_layer(dataset.get().CreateLayer(
            layer_name.c_str(), &dataset.srs().get(), type, m_options.get())) {
    if (!m_layer) {
      throw gdal_error{
          std::string{"failed to create layer '"} + layer_name + "'",
          OGRERR_NONE, dataset.driver_name(), dataset.dataset_name(),
          layer_name};
    }
  }

  OGRLayer& get() noexcept { return *m_layer; }
  const OGRLayer& get() const noexcept { return *m_layer; }

  Dataset& dataset() const noexcept { return m_dataset; }

  const char* name() const { return m_layer->GetName(); }

  Layer& add_field(const std::string& field_name, OGRFieldType type, int width,
                   int precision = 0) {
    ...
    if (m_layer->CreateField(&field) != OGRERR_NONE) {
      throw gdal_error{std::string{"failed to create field '"} + field_name +
                           "' in layer '" + name() + "'",
                       OGRERR_NONE,
  ...
  void create_feature(OGRFeature* feature) {
    ...
    if (result != OGRERR_NONE) {
      throw gdal_error{
          std::string{"creating feature in layer '"} + name() + "' failed",
          result, dataset().driver_name(), dataset().dataset_name()};
    }
  ...
  Layer& start_transaction() {
    ...
    if (result != OGRERR_NONE) {
      throw gdal_error{
          std::string{"starting transaction on layer '"} + name() + "' failed",
          result, m_dataset.driver_name(), m_dataset.dataset_name(), name()};
    }
#endif
    return *this;
  }
  ...
  Layer& commit_transaction() {
    ...
    if (result != OGRERR_NONE) {
      throw gdal_error{std::string{"committing transaction on layer '"} +
                           name() + "' failed",
                       result, m_dataset.driver_name(),
                       m_dataset.dataset_name(), name()};
    }
#endif
    return *this;
  }
...
class Feature {
 ...
 public:
  Feature(Layer& layer, std::unique_ptr<OGRGeometry>&& geometry)
    ...
    if (result != OGRERR_NONE) {
      throw gdal_error{std::string{"setting feature geometry in layer '"} +
                           m_layer.name() + "' failed",
                       result, m_layer.dataset().driver_name(),
                       m_layer.dataset().dataset_name()};
    }
  }

  void add_to_layer() { m_layer.create_feature(m_feature.get()); }
