...
namespace gui {
// static
Lock ThreadManager::callers_lock_;
...
bool ThreadManager::SetMessageLoop(ID identifier, MessageLoop *message_loop) {
  MessageLoop *temp;
  if (GetMessageLoop(identifier, &temp) && temp) {
    return false;
  }

  AutoLock lock(&callers_lock_);
...
bool ThreadManager::PostTask(ID identifier,
  ...
  if (!GetMessageLoop(identifier, &message_loop) || !message_loop) {
    return false;
  }
