...
//////////////////////////////////////////////////////////////////////////
struct SmtGQueryDesc {      //查询 几何条件
  SmtGeometry *pQueryGeom;  //几何条件
  SmtSpatialRs sSRs;        //查询关系
  float select_margin;           //容差
...
struct SmtDataSourceInfo {  //数据源信息
  uint unProvider;
  uint unType;
...
class SmtSpatialIndex {
 public:
  ...
  virtual ~SmtSpatialIndex() { m_pSrcLayer = NULL; }

 public:
  void SetName(const char *szName) {
    sprintf_s(name_, MAX_SPATIALINDEX_NAME, szName);
  }
 ...
 public:
  ...
  virtual long Query(SmtGQueryDesc *pGQueryDesc,
                     SmtVectorLayer *&pQueryResult) = 0;

 protected:
  SmtVectorLayer *m_pSrcLayer;
  char name_[MAX_SPATIALINDEX_NAME];
  uint m_unType;
};
...
class SmtVectorLayer : public SmtLayer {
 ...
 public:
  ...
  //////////////////////////////////////////////////////////////////////////
  // filter
  void SetFilterGeometry(const SmtGeometry *pFilterGeom,
                         bool bIsFilterByGeom = false) {
    m_bIsFilterByGeom = bIsFilterByGeom;
    m_pFilterGeom = pFilterGeom->Clone();
  }

  void SetRectFilter(double dfMinX, double dfMinY, double dfMaxX,
                     double dfMaxY) {
    SmtLinearRing oRing;
    SmtPolygon oPoly;
  ...
  //////////////////////////////////////////////////////////////////////////
  // query
  virtual long Query(const SmtGQueryDesc *pGQueryDesc,
                     const SmtPQueryDesc *pPQueryDesc,
                     SmtVectorLayer *pQueryResult) = 0;

  //////////////////////////////////////////////////////////////////////////
  // feature oper
  virtual long AppendFeature(const SmtFeature *pSmtFeature,
                             bool bClone = false) = 0;

  virtual long AppendFeatureBatch(const SmtFeature *pSmtFeature,
                                  bool bClone = false) = 0;
  ...
  inline SmtFeatureType GetLayerFeatureType(void) const {
    return m_SmtLayerFtType;
  }
...
class SmtTileLayer : public SmtLayer {
 public:
  SmtTileLayer(SmtDataSource *pOwnerDs) : SmtLayer(pOwnerDs), m_lImageCode(-1) {
    ;
  }
...
class SmtDataSource {
 public:
  SmtDataSource(void) { m_bOpen = false; }

  virtual ~SmtDataSource(void) { m_vLayerInfos.clear(); }
 ...
 public:
  ...
  inline void SetName(const char *szName) {
    sprintf_s(m_dsInfo.szName, MAX_DS_NAME, szName);
  }
  inline const char *GetName(void) const { return m_dsInfo.szName; }

  inline void SetUrl(const char *szUrl) {
    sprintf_s(m_dsInfo.szUrl, MAX_URL_LENGTH, szUrl);
  }
  ...
  void GetLayerInfo(SmtLayerInfo &lyrInfo, int index) const {
    ...
    sprintf_s(lyrInfo.szArchiveName, MAX_LAYER_ARCHIVE_NAME,
              m_vLayerInfos[index].szArchiveName);
 ...
 public:
  virtual SmtVectorLayer *CreateVectorLayer(
      const char *szName, fRect &lyrRect,
      SmtFeatureType ftType = SmtFeatureType::SmtFtDot) = 0;
  virtual SmtVectorLayer *OpenVectorLayer(const char *szName) = 0;
  virtual bool DeleteVectorLayer(const char *szName) = 0;

  virtual SmtRasterLayer *CreateRasterLayer(const char *szName, fRect &lyrRect,
                                            long lImageCode) = 0;
  virtual SmtRasterLayer *OpenRasterLayer(const char *szName) = 0;
  virtual bool DeleteRasterLayer(const char *szName) = 0;

  virtual SmtTileLayer *CreateTileLayer(const char *szName, fRect &lyrRect,
                                        long lImageCode) = 0;
