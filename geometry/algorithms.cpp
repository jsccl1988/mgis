...
#define IS_BETWEEN(a, x, b) \
  (((a) <= (x) && (x) <= (b)) || ((b) <= (x) && (x) <= (a)))

namespace core {

//////////////////////////////////////////////////////////////////////////
// PointToPolygon_New1用到
//获取相交点
bool GetCrossing(RawPoint &Crossing, const RawPoint &a1, const RawPoint &a2,
                 const RawPoint &b1, const RawPoint &b2,
                 bool bExactMatch = true);
//获取线段上离已知点最近的点
double GetNearestPointOnLine(const RawPoint &Point, const RawPoint &Ln_A,
                             const RawPoint &Ln_B, RawPoint &Ln_Point,
                             bool bExactMatch = true);
//////////////////////////////////////////////////////////////////////////

//////////////////////////////////////////////////////////////////////////
long GetEnvlope(Envelope &env, RawPoint *pPoints, int nPoint) {
  double dfMinX, dfMinY, dfMaxX, dfMaxY;

  if (nPoint < 2 || NULL == pPoints) return base::ERR_INVALID_PARAM;

  dfMinX = dfMaxX = pPoints[0].x;
  dfMinY = dfMaxY = pPoints[0].y;

  for (int iPoint = 1; iPoint < nPoint; iPoint++) {
    if (dfMaxX < pPoints[iPoint].x) dfMaxX = pPoints[iPoint].x;
    if (dfMaxY < pPoints[iPoint].y) dfMaxY = pPoints[iPoint].y;
    if (dfMinX > pPoints[iPoint].x) dfMinX = pPoints[iPoint].x;
    if (dfMinY > pPoints[iPoint].y) dfMinY = pPoints[iPoint].y;
  }
...
double Distance(RawPoint &pt1, RawPoint *pPoints, int nPoint, int &indexPre,
                int &indexNext) {
  if (nPoint < 2 || NULL == pPoints) return kDoubleMax;

  RawPoint ptPre, ptNext, ptCur(pt1.x, pt1.y);
  float Mintheta = 1.0;
  for (int i = 0; i < nPoint - 1; i++) {
    ...
    float theta = 0.;
    theta = CosABC(ptPre, ptCur, ptNext);
    if (theta < Mintheta) {
      indexPre = i;
      indexNext = i + 1;
      Mintheta = theta;
  ...
  double theta = CosABC(ptPre, ptNext, ptCur), dbfDistance = 0.;
  float sinABC = sqrt(1 - theta * theta);
  Vector2D vBC(ptCur.x - ptNext.x, ptCur.y - ptNext.y);
  dbfDistance = abs(sinABC * vBC.GetLength());

  return dbfDistance;
}

double Distance(RawPoint &pt1, std::vector<RawPoint> &vPoints, int &indexPre,
                int &indexNext) {
  if (vPoints.size() < 2) return kDoubleMax;
...
long Locate(RawPoint &pt1, RawPoint *pPoints, int nPoint, int &indexPre,
            int &indexNext) {
  if (nPoint < 2) return ERR_INVALID_PARAM;

  RawPoint PrePoint, NexPoint;
  ...
  for (int i = starti; i < nPoint - 1; i++) {
    ...
    temp = CosABC(PrePoint, pt1, NexPoint);
...
int GetH(RawPoint A, RawPoint B, RawPoint P,
         RawPoint &H) {  //返回值：-1A延长线上,0在线段端点之间,1在AB延长线上,2
                         // AB重复(此时H无意义)

  int flag = 0;
  if (A.x == B.x && A.y == B.y)
    flag = 2;
  else if (A.x == B.x) {  //平行于y轴
    H.x = A.x;
    H.y = P.y;
    float m, n, o;
    m = (P.y - A.y);
...
long Locate(RawPoint &pt1, std::vector<RawPoint> &vPoints, int &indexPre,
            int &indexNext) {
  if (vPoints.size() < 2) return ERR_INVALID_PARAM;
...
double Distance_New(RawPoint &Point, RawPoint *pPoints, int nPoint,
                    int &indexPre, int &indexNext) {
  if (nPoint < 2 || NULL == pPoints) return kDoubleMax;

  int i;
  double d, Distance;
  RawPoint *pA, *pB, pt, Next;
  ...
  Distance = GetNearestPointOnLine(Point, *pA, *pB, Next);

  for (i = 1; i < nPoint && Distance != 0.0; i++, pB = pA++) {
    if ((d = GetNearestPointOnLine(Point, *pA, *pB, pt)) >= 0.0 &&
...
double Distance_New(RawPoint &Point, std::vector<RawPoint> &vPoints,
                    int &indexPre, int &indexNext) {
  if (vPoints.size() < 2) return kDoubleMax;
  ...
  std::vector<RawPoint>::iterator iterA, iterB;
  iterB = vPoints.begin();
  iterA = iterB + 1;

  Distance = GetNearestPointOnLine(Point, *iterA, *iterB, Next);

  for (i = 1; i < vPoints.size() && Distance != 0.0; i++, iterB = iterA++) {
    if ((d = GetNearestPointOnLine(Point, *iterA, *iterB, pt)) >= 0.0 &&
        (d < Distance || Distance < 0.0)) {
      Distance = d;
      Next = pt;
      indexPre++;
...
long PointToLineSect(RawPoint &C, RawPoint &A, RawPoint &B) {
  /*
  B ^
  |
  P1	   |        P2
  ...
  */
  ...
  Vector2D v1(B.x - A.x, B.y - A.y), v2(C.x - A.x, C.y - A.y);
...
long PointToPolygon(RawPoint &pt, RawPoint *pPoints, int nPoint) {
  if (nPoint < 3 || NULL == pPoints) return -2;

  int i, n = nPoint;
  double cos0;
  double flag = 0;  //用于累加角度
  double m;
  RawPoint ptPre, ptNext;
  for (i = n - 2; i >= 0; i--) {
    ptPre = pPoints[i];
    ptNext = pPoints[i + 1];

    //叉积判断点是否在线段
    Vector2D v1(ptNext.x - ptPre.x, ptNext.y - ptPre.y),
        v2(pt.x - ptPre.x, pt.y - ptPre.y);
...
long PointToPolygon_New(RawPoint &pt, RawPoint *pPoints, int nPoint) {
  if (nPoint < 3 || NULL == pPoints) return -2;

  int i, j, k, wn = 0;
  int n = nPoint;
  for (i = n - 1, j = 0; j < n; i = j, j++) {
    k = (pt.x - pPoints[i].x) * (pPoints[j].y - pPoints[i].y) -
        (pPoints[j].x - pPoints[i].x) * (pt.y - pPoints[i].y);
    if ((pt.y >= pPoints[i].y && pt.y <= pPoints[j].y) ||
        (pt.y <= pPoints[i].y && pt.y >= pPoints[j].y)) {
      if (k < 0)
        wn++;
      else if (k > 0)
        wn--;
        ...
        if ((pt.y <= pPoints[i].y && pt.y >= pPoints[j].y &&
             pt.x <= pPoints[i].x && pt.x >= pPoints[j].x) ||
            (pt.y <= pPoints[i].y && pt.y >= pPoints[j].y &&
             pt.x >= pPoints[i].x && pt.x <= pPoints[j].x) ||
            (pt.y >= pPoints[i].y && pt.y <= pPoints[j].y &&
             pt.x <= pPoints[i].x && pt.x >= pPoints[j].x) ||
...
long PointToPolygon_New1(RawPoint &pt, RawPoint *pPoints, int nPoint) {
  if (nPoint < 3 || NULL == pPoints) return -2;

  Envelope env;
  GetEnvlope(env, pPoints, nPoint);

  if (nPoint > 2 /*&& env.Contains(pt.x,pt.y)*/) {
    int nCrossings;
    RawPoint A, B, C, *pA, *pB;
    ...
    for (int iPoint = 0, goNext = 0; iPoint < nPoint; iPoint++, pB = pA++) {
      if (pA->y != pB->y) {
        ...
        {
          if (((goNext > 0 && pA->y > pB->y) ||
               (goNext < 0 && pA->y < pB->y)) &&
              pB->x <= B.x)
            nCrossings++;

          goNext = 0;
        } else if (((pB->y < pt.y && pt.y <= pA->y) ||
                    (pB->y > pt.y && pt.y >= pA->y)) &&
                   (pB->x < pt.x || pA->x < pt.x)) {
          if (GetCrossing(C, *pA, *pB, A, B)) {
            nCrossings++;
          }
        }
      }
...
bool GetCrossing(RawPoint &Crossing, const RawPoint &a1, const RawPoint &a2,
                 const RawPoint &b1, const RawPoint &b2, bool bExactMatch) {
  double lambda, div, a_dx, a_dy, b_dx, b_dy;

  if (bExactMatch && ((max(a1.x, a2.x) < min(b1.x, b2.x)) ||
...
double GetNearestPointOnLine(const RawPoint &Point, const RawPoint &Ln_A,
  ...
  if (GetCrossing(Ln_Point, Ln_A, Ln_B, Point, Point_B, false)) {
    if (!bExactMatch ||
        (bExactMatch && IS_BETWEEN(Ln_A.x, Ln_Point.x, Ln_B.x) &&
         IS_BETWEEN(Ln_A.y, Ln_Point.y, Ln_B.y))) {
...
//////////////////////////////////////////////////////////////////////////
double CalcPolygonArea(RawPoint *pPoints, int nPoint) {
  double dfAreaSum = 0.0;
  int i;

  if (pPoints == NULL || nPoint < 2) return 0;
...
double CalcTriArea(RawPoint A, RawPoint B, RawPoint C) {
  return (0.5 * ((B.x - A.x) * (C.y - A.y) - (B.y - A.y) * (C.x - A.x)));
}

//////////////////////////////////////////////////////////////////////////
double CosABC(RawPoint A, RawPoint B, RawPoint C) {
  double theta;
  Vector2D BA(A.x - B.x, A.y - B.y), BC(C.x - B.x, C.y - B.y);
...
//////////////////////////////////////////////////////////////////////////
long Interpolate_Lugrange(RawPoint *&pPnts, int &nPnts, RawPoint *pCtrlPnts,
                          int nCtrlPnts, int nSection) {
  ...
  //释放
  for (int i = 0; i < 4; i++) {
    SAFE_DELETE_A(ppfBlend[i]);
    SAFE_DELETE_A(ppBlend[i]);
    SAFE_DELETE_A(pplBlend[i]);
  }

  SAFE_DELETE_A(ppfBlend);
  SAFE_DELETE_A(ppBlend);
  SAFE_DELETE_A(pplBlend);

  return ERR_NONE;
}

long Interpolate_Geometry(RawPoint *&pPnts, int &nPnts, RawPoint *pCtrlPnts,
                          int nCtrlPnts, float dt) {
  if (NULL == pCtrlPnts || nCtrlPnts < 4 || dt < 0.) return ERR_INVALID_PARAM;

  int i, j, k, n;
  float t;
  RawPoint *q = NULL, *r = NULL;

  q = new RawPoint[nCtrlPnts];
  r = new RawPoint[nCtrlPnts];
  ...
  pPnts[0] = pCtrlPnts[0];
  for (t = 0; t <= 1; t += dt) {
    for (i = 0; i < nCtrlPnts; i++) r[i] = pCtrlPnts[i];
...
long Interpolate_BSpline(RawPoint *&pPnts, int &nPnts, RawPoint *pCtrlPnts,
                         int nCtrlPnts, float dt) {
  if (NULL == pCtrlPnts || nCtrlPnts < 4 || dt < 0.) return ERR_INVALID_PARAM;
...
long Interpolate_3Spline(RawPoint *&pPnts, int &nPnts, RawPoint *pCtrlPnts,
                         int nCtrlPnts, int nInterCount) {
...
bool LineSegOverLineSeg(const float &x1, const float &y1, const float &x2,
                        const float &y2, const float &u1, const float &v1,
...
bool PointInTri(const RawPoint &v, const RawPoint &v1, const RawPoint &v2,
                const RawPoint &v3) {  //看v是否在三角形v1,v2,v3内
  //作直线(v[X],v[Y])-(inf,v[Y])看是否与线段v1v2,线段v2v3,线段v3v4中恰一个有交点
  bool over12 = LineSegOverLineSeg(v.x, v.y, INF, v.y, v1.x, v1.y, v2.x, v2.y);
  bool over23 = LineSegOverLineSeg(v.x, v.y, INF, v.y, v2.x, v2.y, v3.x, v3.y);
  bool over31 = LineSegOverLineSeg(v.x, v.y, INF, v.y, v3.x, v3.y, v1.x, v1.y);

  if (over12 + over23 + over31 == 1) {  //说明在三角形内
    return true;
  }
  return false;
}

double IsCCW(RawPoint &a, RawPoint &b, RawPoint &c) {
  return ((b.x - a.x) * (c.y - b.y) - (c.x - b.x) * (b.y - a.y));
}

long DivPolygenIntoTriMesh(vector<Triangle> &trilist, RawPoint *pPoints,
                           int nPoint)
//将多边形切割成三角网格
...
//对自相交多边形和逆时针多边形所得IDtriList为空，前端返回false
{
  //如果顶点数不足三个，直接退出
  nPoint--;
  if (NULL == pPoints || nPoint < 3) return ERR_INVALID_PARAM;
  ...
  for (int i = 0; i <= n - 1; i++) {
    int i1 = i;
    int i2 = (i + 1) % n;
    const RawPoint &v1 = pPoints[i1];
    const RawPoint &v2 = pPoints[i2];
    //线段v1v2与其后各边进行相交检测
    for (int j = i2; j <= n - 1; j++) {
      int j1 = j;
      int j2 = (j + 1) % n;
      if (j1 == i2 || j2 == i1) continue;  //跳过相邻边

      const RawPoint &u1 = pPoints[j1];
      const RawPoint &u2 = pPoints[j2];
      //对v1v2与u1u2进行相交检测
      if (LineSegOverLineSeg(v1.x, v1.y, v2.x, v2.y, u1.x, u1.y, u2.x, u2.y)) {
        return ERR_FAILURE;
      }
    }
  }  //一定无自相交
  //顺时针层层削减掉非凹角
  vector<RawPoint> tvlist;  //削减后剩余的顶点列表
  vector<int> vIndex;
  ...
  while (1) {
    ...
    int i2 = (i + 1) % n;
    int i3 = (i + 2) % n;
    RawPoint &v1 = tvlist[i1];
    RawPoint &v2 = tvlist[i2];
    RawPoint &v3 = tvlist[i3];
    ...
    for (int j = 0; j <= n - 1; j++) {
      ...
      RawPoint &v = tvlist[j];
      //看v是否在三角形v1,v2,v3内
      if (PointInTri(v, v1, v2, v3)) {
        isInTri = true;
        break;
      }
    }  //得到isInTri

    if (isHeaveOrEven && !isInTri) {  //可以削减掉三角形i1,i2,i3
      Triangle tri;
      tri.a = vIndex[i3];
      tri.b = vIndex[i2];
      tri.c = vIndex[i1];
      ...
      if ((int)tvlist.size() == 2) {  //如果只剩两点了
        //切割完毕，返回
        //调整顺序
        for (int i = 0; i < trilist.size(); i++) {
          if (IsCCW(pPoints[trilist[i].a], pPoints[trilist[i].b],
                    pPoints[trilist[i].c]) > 0) {
    ...
    } else {  //不可削减掉三角形i1,i2,i3
      i++;
      count++;
      if (count ==
          (int)tvlist
              .size()) {  //如果已连续削减一周都没有能削减下来，说明不可削减
        trilist.resize(0);
