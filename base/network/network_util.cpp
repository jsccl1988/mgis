...
bool GetMacAddressByNetBIOS(std::string &mac_address) {
  ...
  for (i = 0; i < lenum.length; ++i) {
    ...
    uRetCode = Netbios(&Ncb);
    if (uRetCode == 0) {
      if (Adapter.adapt.adapter_address[0] + Adapter.adapt.adapter_address[1] +
...
bool GetMacAddressBySNMP(std::string &mac_address) {
  ...
  f_SnmpExtensionQuery(ASN_RFC1157_GETNEXTREQUEST, &var_bind_list, &errorStatus,
                       &errorIndex);
  var_bind_list.len = 2;
  ...
  for (int j = 0; j < var_bind[0].value.asnValue.number; j++) {
    ...
    // We can expect this call to succeed a # of times corresponding to the # of
    // adapters reported to be in the system
    if (f_SnmpExtensionQuery(ASN_RFC1157_GETNEXTREQUEST, &var_bind_list,
                             &errorStatus, &errorIndex) == FALSE)
      continue;
    // Confirm that the proper type has been returned
    if (SnmpUtilOidNCmp(&var_bind[0].name, &MIB_ifEntryType,
                        MIB_ifEntryType.idLength) != 0)
      continue;
    // Type 6 describes ethernet interfaces
    if (var_bind[0].value.asnValue.number != 6) continue;
    // Confirm that we have an address here
    if (SnmpUtilOidNCmp(&var_bind[1].name, &MIB_ifMACEntAddr,
                        MIB_ifMACEntAddr.idLength) != 0)
      continue;
    if (var_bind[1].value.asnValue.address.stream == NULL) continue;
...
bool GetMacAddressByAdaptersInfo(std::string &mac_address) {
  ...
  if (GetAdaptersInfo(adapter_info, &out_buf_len) == NO_ERROR) {
    ...
    for (; adapter != NULL; adapter = adapter->Next) {
      ...
      StringPrintf(mac_address, "%02x-%02x-%02x-%02x-%02x-%02x",
                   int(adapter->Address[0]), int(adapter->Address[1]),
                   int(adapter->Address[2]), int(adapter->Address[3]),
                   int(adapter->Address[4]), int(adapter->Address[5]));
...
bool GetMacAddressByAdaptersAddresses(std::string &mac_address) {
  ...
  if (GetAdaptersAddresses(AF_UNSPEC, 0, NULL, addresses, &out_buf_len) ==
  ...
  if (GetAdaptersAddresses(AF_UNSPEC, 0, NULL, addresses, &out_buf_len) ==
    ...
    for (; curr_addresses != NULL; curr_addresses = curr_addresses->Next) {
      if (curr_addresses->PhysicalAddressLength != 6) continue;
...
bool GetMacAddress(std::string &mac_address) {
  ...
  if (true == (success = GetMacAddressByNetBIOS(mac_address))) {
    ...
    return true;
             ...
             (success = GetMacAddressByAdaptersAddresses(mac_address))) {
    return true;
  } else {
  ...
  if (success) {
    ...
    while (cursor != 0) {
      if ((cursor->ifa_addr->sa_family == AF_LINK) &&
          (((const struct sockaddr_dl *)cursor->ifa_addr)->sdl_type ==
           IFT_ETHER)) {
        dl_addr = (const struct sockaddr_dl *)cursor->ifa_addr;
        const unsigned char *base =
            (const unsigned char *)&dl_addr->sdl_data[dl_addr->sdl_nlen];
...
bool AnalyzeNetAddress(const std::string &address, uint32_t &out_ip,
                       uint16_t &out_port) {
  std::list<std::string> ip_and_port;
  StringTokenize(address, ":", ip_and_port);
  if (ip_and_port.size() != 2) return false;
