...
#if defined(OS_WIN)
  if (err == WSAEWOULDBLOCK || err == WSAEINPROGRESS) return true;
#else
  if (err == EWOULDBLOCK || err == EINPROGRESS || err == EAGAIN) return true;
#endif
  return false;
}
...
inline void set_block(SOCKET s, bool b) {
  ...
  if (b) {
    if (val & O_NONBLOCK) fcntl(s, F_SETFL, val & (~O_NONBLOCK));
  } else {
    if (!(val & O_NONBLOCK)) fcntl(s, F_SETFL, val | O_NONBLOCK);
  }
#endif
...
inline void reuse_address(SOCKET s) {
  int v = 1;
  int r =
      ::setsockopt(s, SOL_SOCKET, SO_REUSEADDR, (const char *)&v, sizeof(v));
  if (r != 0) {
    int err = socket_error();
    // use trace here?
...
inline void allow_broadcast(SOCKET s) {
  int v = 1;
  int r =
      ::setsockopt(s, SOL_SOCKET, SO_BROADCAST, (const char *)&v, sizeof(v));
  if (r != 0) {
    int err = socket_error();
    // use trace here?
