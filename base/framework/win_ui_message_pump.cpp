...
WinUIMessagePump::WinUIMessagePump() { InitMessageWnd(); }
...
void WinUIMessagePump::ScheduleWork() {
  if (::InterlockedExchange(&have_work_, 1)) return;
...
LRESULT CALLBACK WinUIMessagePump::WndProcThunk(HWND hwnd, UINT message,
...
bool WinUIMessagePump::ProcessMessageHelper(const MSG& msg) {
  ...
  if (::CallMsgFilter(const_cast<MSG*>(&msg), kMessageFilterCode)) return true;
