...
MessageLoop::MessageLoop()
  ...
  // 默认消息循环
  if (type_ == kDefaultMessageLoop) pump_.reset(new DefaultMessagePump);
...
MessageLoop *MessageLoop::current() { return g_lazy_ptr.Pointer()->Get(); }

#if defined(OS_WIN)

UIMessageLoop *MessageLoop::ToUIMessageLoop() {
  if (type_ == kUIMessageLoop) return reinterpret_cast<UIMessageLoop *>(this);
  return NULL;
}

IOMessageLoop *MessageLoop::ToIOMessageLoop() {
  if (type_ == kIOMessageLoop) return reinterpret_cast<IOMessageLoop *>(this);
  return NULL;
}
...
bool MessageLoop::DeletePendingTasks() {
  ...
  while (!work_queue_.empty()) {
    ...
    work_queue_.pop();
    if (!task.delayed_run_time.is_null()) AddToDelayedWorkQueue(task);
  }
  ...
  while (!delayed_work_queue_.empty()) delayed_work_queue_.pop();

  return has_work;
}

void MessageLoop::Quit() {
  if (state_) state_->quit_received = true;
}
...
bool MessageLoop::DoWork() {
  // 任务当前是否允许被执行
  if (!nestable_tasks_allowed_) return false;
  ...
  for (;;) {
    ...
    do {
      ...
      if (!task.delayed_run_time.is_null()) {
        ...
        if (DeferOrRunPendingTask(task)) return true;
      }
    } while (!work_queue_.empty());
  }
...
bool MessageLoop::ProcessNextDelayedNonNestableTask() {
  // 嵌套任务？
  if (state_->run_depth != 1) return false;
...
MessageLoop::AutoRunState::~AutoRunState() { loop_->state_ = previous_state_; }
...
bool IOMessageLoop::WaitForIOCompletion(DWORD timeout, IOHandler *handler) {
  assert(this == current());
  return static_cast<IOMessagePump *>(pump())->WaitForIOCompletion(timeout,
                                                                   handler);
}

#elif defined(OS_POSIX)
bool IOMessageLoop::WatchFileDescriptor(int fd, bool persistent, Mode mode,
                                        FileDescriptorWatcher *controller,
                                        Watcher *delegate) {
  assert(this == current());
  return static_cast<IOMessagePump *>(pump())()->WatchFileDescriptor(
      fd, persistent, static_cast<LibeventMessagePump::Mode>(mode), controller,
      delegate);
}
#endif
