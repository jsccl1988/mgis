...
class AutoLazyEraser {
 public:
  AutoLazyEraser(LazyEraser *lazy_eraser) : ptr_(lazy_eraser) {
    if (ptr_) ptr_->AquireLazyErase();
  }

  AutoLazyEraser(AutoLazyEraser &lazy_eraser) {
    if (lazy_eraser.ptr_) lazy_eraser.ptr_->AquireLazyErase();
  ...
  ~AutoLazyEraser() {
    if (ptr_) {
      if (ptr_->ReleaseLazyErase() < 1) ptr_->Compact();
    }
  }
...
class ObserverList : public LazyEraser {
 public:
  ...
  void RemoveObserver(ObserverType *observer) {
    typename std::vector<ObserverType *>::iterator pos =
        std::find(observers_.begin(), observers_.end(), observer);
    if (pos == observers_.end()) return;
    if (lazy_erase_ > 0)
      *pos = NULL;
    else
  ...
  void Compact() {
    if (lazy_erase_ > 0 || observers_.empty()) return;
  ...
  ObserverType *GetObserver(size_t index) const {
    if (index >= GetObserverCount()) return NULL;
