...
template <typename BlockAllocator, unsigned MaxBlocks>
inline bool BlockBuffer<BlockAllocator, MaxBlocks>::append(const char *app,
                                                           size_t len) {
...
template <typename BlockAllocator, unsigned MaxBlocks>
inline bool BlockBuffer<BlockAllocator, MaxBlocks>::replace(size_t pos,
                                                            const char *rep,
                                                            size_t n) {
  ...
  if (n > 0) memmove(data_ + pos, rep, n);
  return true;
}

template <typename BlockAllocator, unsigned MaxBlocks>
inline void BlockBuffer<BlockAllocator, MaxBlocks>::erase(size_t pos, size_t n,
                                                          bool hold) {
 ...
 */
template <typename BlockAllocator, unsigned MaxBlocks>
inline bool BlockBuffer<BlockAllocator, MaxBlocks>::increase_capacity(
  ...
  // if (increase_size > maxsize() - free) throw std::out_of_range("BlockBuffer
  // out of range");
...
class ForwardBuffer {
 public:
  ...
  ~ForwardBuffer() {
    if (buffer_) buffer_->erase(0, position_);
  }
