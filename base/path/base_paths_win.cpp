...
bool DirectoryAsDLLSearchPath(const PathString& path) {
  ...
  if (hKernel32) {
#ifdef _UNICODE
    pfnCurrentDirectory = (SetCurrentDirectoryFnPtr)::GetProcAddress(
        hKernel32, "SetDllDirectoryW");
#else
    pfnCurrentDirectory = (SetCurrentDirectoryFnPtr)::GetProcAddress(
        hKernel32, "SetDllDirectoryA");
#endif  // _UNICODE
...
bool GetCurrentDirectory(PathString* dir) {
  ...
  PathString dir_str(system_buffer);
  *dir = dir_str;
  return true;
}
...
bool PathProvider(int key, PathString* result) {
  ...
  switch (key) {
    ...
    case base::DIR_USER_QUICK_LAUNCH:
      if (!PathProvider(base::DIR_APP_DATA, &cur)) return false;
    ...
    case base::DIR_TASKBAR_PINS:
      if (!PathProvider(base::DIR_USER_QUICK_LAUNCH, &cur)) return false;
