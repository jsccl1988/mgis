...
bool SetObjectToLowIntegrity(HANDLE hObject,
  ...
  if (!IsRunningOnVistaOrHigher()) return true;
  ...
  if (::ConvertStringSecurityDescriptorToSecurityDescriptorW(
          kLowIntegeritySddlSaclW, SDDL_REVISION_1, &pSD, NULL)) {
...
bool IsUserAdmin() {
  ...
  BOOL result = ::AllocateAndInitializeSid(
      ...
      0, 0, 0, 0, 0, &administrators_group);
  if (result) {
...
bool IsUserNonElevatedAdmin() {
  ...
  if (!IsRunningOnVistaOrHigher()) return false;
...
HRESULT GetSidIntegrityLevel(PSID sid, MANDATORY_LEVEL* level) {
  ...
  if (!count || *count != 1) return E_FAIL;

  DWORD* rid = ::GetSidSubAuthority(sid, 0);
  if (!rid) return E_FAIL;
...
HRESULT GetProcessIntegrityLevel(DWORD process_id, MANDATORY_LEVEL* level) {
  if (!IsRunningOnVistaOrHigher()) return E_NOTIMPL;
  ...
  if (process != NULL) {
    ...
    if (::OpenProcessToken(process, TOKEN_QUERY | TOKEN_QUERY_SOURCE,
      ...
      ::GetTokenInformation(current_token, TokenIntegrityLevel, NULL, 0,
                            &label_size);
      if (label_size && (label = reinterpret_cast<TOKEN_MANDATORY_LABEL*>(
                             ::LocalAlloc(LPTR, label_size))) != NULL) {
        if (::GetTokenInformation(current_token, TokenIntegrityLevel, label,
