...
bool ThreadMap::AquireAccess() {
  ...
  if (!tls || tls->managed < 1) return false;
  return true;
}

bool ThreadMap::RegisterThread(int self_identifier) {
  if (self_identifier < 0) return false;

  FrameworkThreadTlsData *tls = FrameworkThread::GetTlsData();
  // should be called by a Framework thread
  ...
  if (!pr.second) {
    if (pr.first->second != tls->self) {
      // another thread has registered with the same id
      return false;
    }
...
FrameworkThread *ThreadMap::QueryThreadInternal(int identifier) const {
  ...
  if (iter == GetInstance()->threads_.end()) return NULL;
  return iter->second;
}
...
int ThreadMap::QueryThreadId(const FrameworkThread *thread) {
  ...
  for (iter = GetInstance()->threads_.begin();
    ...
    if (iter->second == thread) return iter->first;
  }
  return -1;
}
...
std::shared_ptr<MessageLoopProxy> ThreadMap::GetMessageLoop(
  ...
  if (message_loop == NULL) return NULL;
...
FrameworkThread *ThreadManager::CurrentThread() {
  FrameworkThreadTlsData *tls = FrameworkThread::GetTlsData();
  if (!tls || tls->managed < 1) return NULL;
  return tls->self;
}
...
void ThreadManager::PostRepeatedTask(int thread_id,
                                     ...
                                     const TimeDelta &delay, int times) {
  StdClosure closure = base::Bind(&ThreadManager::RunRepeatedly2, thread_id,
                                   task, delay, times);
