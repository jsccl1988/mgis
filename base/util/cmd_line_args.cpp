...
CmdLineArgs::CmdLineArgs(const wchar_t *command_line) : buffer_(NULL) {
  size_t alloc;
  size_t length;

#if defined(OS_WIN)
  if (command_line == NULL) command_line = ::GetCommandLineW();
#endif  // OS_WIN
  ...
  while (*ch != Term) {
    ...
    while (*ch != Term) {
      if (isQuoted) {
        ...
        if (iswspace(*ch)) break; /* a common argument found */
      }
      ch++;
    }
