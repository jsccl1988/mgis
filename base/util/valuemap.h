...
class BASE_EXPORT ValueMap {
 ...
 private:
  struct Item {
    ...
    Item(Type t, void *v, uint8_t a) : access_(a), type_(t), ptr_(v) {
      assert(ptr_);
    }
    Item(const Item &src) {
      access_ = src.access_;
 ...
 public:
  ...
  ~ValueMap() {
    items_.clear();
    for (size_t i = 0; i < types_num; ++i) delete formats_[i];
  }
  void clear() { items_.clear(); }
  void bind(const std::string &name, void *ptr, Type type,
            uint8_t access = access_read) {
    assert(ptr);
    assert(type >= t_char && type < types_num);
    items_[name] = Item(type, ptr, access);
  }
  void bind(const char *name, void *ptr, Type type,
            uint8_t access = access_read) {
  ...
  bool set(const std::string &name, const char *value) {
    ...
    if (i.type_ == t_string || i.type_ == t_pointer) return false;
    if (!(i.access_ & access_write)) return false;
  ...
  void enumerate(std::vector<std::string> &names) const {
    ...
    for (; it != items_.end(); ++it) names.push_back(it->first);
  }
};
