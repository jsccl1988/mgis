...
int StringTokenizeT(const std::basic_string<CharType> &input,
  ...
  for (token_begin = token_end = 0;
       token_end != std::basic_string<CharType>::npos;) {
    token_begin = input.find_first_not_of(delimitor, token_begin);
    if (token_begin == std::basic_string<CharType>::npos) break;
...
size_t StringReplaceAllT(const std::basic_string<CharType> &find,
   ...
   * we use two passes to finish the task in the case that replace.size() is
   * greater find.size()
   */
  ...
  if (find_length < replace_length) {
    ...
    for (;;) {
      ...
      if (offset == std::basic_string<CharType>::npos) break;
      replaced++;
      offset += find_length;
    }
    ...
    memmove((void *)(data_ptr + offset), (void *)data_ptr,
  ...
  while (offset < endpos) {
    ...
    if (found_pos != std::basic_string<CharType>::npos) {
      ...
      if (target != found_pos)
        memmove((void *)(data_ptr + target), (void *)(data_ptr + offset),
                (found_pos - offset) * sizeof(CharType));

      target += found_pos - offset;

      /* replace */
      memcpy(data_ptr + target, replace.data(),
      ...
      /* ending work  */
      if (target != offset)
        memcpy((void *)(data_ptr + target), (void *)(data_ptr + offset),
...
inline int vsnprintfT(wchar_t *dst, size_t count, const wchar_t *format,
                      va_list ap) {
#if defined(OS_WIN)
...
template <typename CharType>
void StringAppendVT(const CharType *format, va_list ap,
                    std::basic_string<CharType> &output) {
  ...
  int result =
      vsnprintfT(stack_buffer, COUNT_OF(stack_buffer), format, ap_copy);
  va_end(ap_copy);
...
void StringTrimT(std::basic_string<CharType> &output) {
  ...
  for (; bound2 > 0; bound2--)
    if (NOT_SPACE(src[bound2 - 1])) break;

  for (; bound1 < bound2; bound1++)
    if (NOT_SPACE(src[bound1])) break;
...
void StringTrimRightT(std::basic_string<CharType> &output) {
  ...
  for (; length > 0; length--)
    if (NOT_SPACE(src[length - 1])) break;
...
std::list<std::string> StringTokenize(const char *input,
                                      const char *delimitor) {
  std::list<std::string> output;
  std::string input2(input);

  if (input2.empty()) return output;
...
std::list<std::wstring> StringTokenize(const wchar_t *input,
                                       const wchar_t *delimitor) {
...
int StringTokenize(const std::string &input, const std::string &delimitor,
                   std::list<std::string> &output) {
  return StringTokenizeT<char>(input, delimitor, output);
}

int StringTokenize(const std::wstring &input, const std::wstring &delimitor,
                   std::list<std::wstring> &output) {
  return StringTokenizeT<wchar_t>(input, delimitor, output);
}

size_t StringReplaceAll(const std::string &find, const std::string &replace,
                        std::string &output) {
  return StringReplaceAllT<char>(find, replace, output);
}

size_t StringReplaceAll(const std::wstring &find, const std::wstring &replace,
                        std::wstring &output) {
...
void LowerString(std::wstring &str) {
  ...
  for (; start < end; start++) {
    if (*start >= L'A' && *start <= L'Z') *start += L'a' - L'A';
  }
}
...
std::basic_string<UTF32Char> UTF8ToUTF32(const UTF8Char *utf8, size_t length) {
  ...
  while (src_begin < src_end) {
    ConversionResult result =
        ConvertUTF8toUTF32(&src_begin, src_end, &dst_begin,
                           dst_begin + COUNT_OF(output), lenientConversion);
...
std::string UTF32ToUTF8(const UTF32Char *utf32, size_t length) {
  ...
  while (src_begin < src_end) {
    ConversionResult result =
        ConvertUTF32toUTF8(&src_begin, src_end, &dst_begin,
                           dst_begin + COUNT_OF(output), lenientConversion);
...
std::basic_string<UTF32Char> UTF16ToUTF32(const UTF16Char *utf16,
                                          size_t length) {
  UTF32Char output[4096];
  const UTF16 *src_begin = reinterpret_cast<const UTF16 *>(utf16);
  const UTF16 *src_end = src_begin + length;
  ...
  while (src_begin < src_end) {
    ConversionResult result =
        ConvertUTF16toUTF32(&src_begin, src_end, &dst_begin,
                            dst_begin + COUNT_OF(output), lenientConversion);
...
std::wstring UTF32ToUTF16(const UTF32Char *utf32, size_t length) {
  ...
  while (src_begin < src_end) {
    ConversionResult result =
        ConvertUTF32toUTF16(&src_begin, src_end, &dst_begin,
                            dst_begin + COUNT_OF(output), lenientConversion);
...
bool ValidateGB2312Stream(const void *stream, unsigned length) {
  ...
  for (; s < e; s++) {
    if (*s < 0x80) continue;
    if (*s < 0xA1 || 0xE7 < *s) break;
    if (++s == e) return false;
    if (*s < 0xA1 || 0xFE < *s) break;
  }
...
bool ValidateGBKStream(const void *stream, unsigned length) {
  ...
  for (; s < e; s++) {
    if (*s < 0x80) continue;
    if (*s < 0x81 || 0xFE < *s) break;
    if (++s == e) return false;
    if (*s < 0x40 || 0xFE < *s) break;
  }
...
bool ValidateBIG5Stream(const void *stream, unsigned length) {
  ...
  for (; s < e; s++) {
    if (*s < 0x80) continue;
    if (*s < 0xA1 || 0xF9 < *s) break;
    if (++s == e) return false;
    if (*s < 0x40 || 0xFE < *s || (0x7E < *s && *s < 0xA1)) break;
  }
