...
BASE_EXPORT const std::string &StringPrintf(std::string &output,
                                            const char *format, ...);
BASE_EXPORT const std::wstring &StringPrintf(std::wstring &output,
                                             const wchar_t *format, ...);
BASE_EXPORT void StringPrintfV(std::string &output, const char *format,
                               va_list ap);
BASE_EXPORT void StringPrintfV(std::wstring &output, const wchar_t *format,
                               va_list ap);
...
BASE_EXPORT void StringAppendF(std::wstring &output, const wchar_t *format,
                               ...);
BASE_EXPORT void StringAppendV(std::string &output, const char *format,
                               va_list ap);
BASE_EXPORT void StringAppendV(std::wstring &output, const wchar_t *format,
                               va_list ap);

// find all tokens splitted by one of the characters in 'delimitor'
BASE_EXPORT std::list<std::string> StringTokenize(const char *input,
                                                  const char *delimitor);
BASE_EXPORT std::list<std::wstring> StringTokenize(const wchar_t *input,
                                                   const wchar_t *delimitor);
BASE_EXPORT int StringTokenize(const std::string &input,
                               const std::string &delimitor,
                               std::list<std::string> &output);
BASE_EXPORT int StringTokenize(const std::wstring &input,
                               const std::wstring &delimitor,
                               std::list<std::wstring> &output);

// replace all 'find' with 'replace' in the string
BASE_EXPORT size_t StringReplaceAll(const std::string &find,
                                    const std::string &replace,
                                    std::string &output);
BASE_EXPORT size_t StringReplaceAll(const std::wstring &find,
                                    const std::wstring &replace,
                                    std::wstring &output);

// print the binary data in a human-readable hexadecimal format
BASE_EXPORT void BinaryToHexString(const void *binary, size_t length,
                                   std::string &output);
...
// the functions will try to convert the characters in input stream until the
// end of the stream or an error occurs
BASE_EXPORT int HexStringToBinary(const char *input, size_t length,
                                  std::string &output);
...
// the following functions are used to convert encodings in utf-8、utf-16 and
// utf-32
...
BASE_EXPORT std::basic_string<UTF32Char> UTF8ToUTF32(const UTF8Char *utf8,
                                                     size_t length);
BASE_EXPORT std::string UTF32ToUTF8(const UTF32Char *utf32, size_t length);
BASE_EXPORT std::basic_string<UTF32Char> UTF16ToUTF32(const UTF16Char *utf16,
                                                      size_t length);
...
BASE_EXPORT std::basic_string<UTF32Char> UTF16ToUTF32(
    const std::wstring &utf16);
BASE_EXPORT std::wstring UTF32ToUTF16(
    const std::basic_string<UTF32Char> &utf32);

// the following functions are used to validate encoded streamings, such as
// utf-8, gbk, big5, etc.
