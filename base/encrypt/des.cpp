...
const static char Shift_Table[16] = {1, 1, 2, 2, 2, 2, 2, 2,
                                     1, 2, 2, 2, 2, 2, 2, 1};
...
const static char E_Table[48] = {32, 1,  2,  3,  4,  5,  4,  5,  6,  7,  8,  9,
...
std::string yxDES::Decrypt(const std::string& src, const char* key, int iMode,
                           int iPKCS) {
  std::string des;
...
std::string yxDES::Encrypt(const std::string& src, const char* key, int iMode,
                           int iPKCS) {
  std::string des;
...
void yxDES::FunctionF(char* sz_Li, char* sz_Ri, unsigned int iKey,
                      unsigned int keyN) {
  char sz_48R[48] = {0};
  char sz_xor48[48] = {0};
  char sz_P32[32] = {0};
...
void yxDES::XOR(char* szParam1, char* szParam2, unsigned int uiParamLength,
...
void yxDES::Bytes2Bits(const char* srcBytes, char* dstBits,
...
void yxDES::Int2Bits(unsigned int _src, char* dstBits) {
  for (unsigned int i = 0; i < 4; i++) dstBits[i] = ((_src << i) & 8) >> 3;
}
...
char* yxDES::GetCiphertextInBytes() { return szCiphertextInBytes; }
...
char* yxDES::GetCiphertextAnyLength() { return szFCiphertextAnyLength; }
...
void yxDES::EncryptAnyLength(char* _srcBytes, unsigned int _bytesLength,
                             unsigned int keyN) {
...
void yxDES::DecryptAnyLength(char* _srcBytes, unsigned int _bytesLength,
                             unsigned int keyN) {
  if (_bytesLength == 8) {
    ...
    if (iResidue != 0) {
      ...
      DecryptData(szLast8Bits, keyN);
      memcpy(szFPlaintextAnyLength + (iParts << 3), szPlaintextInBytes,
             iResidue);
    }
    szFPlaintextAnyLength[_bytesLength] = '\0';
  }
...
void yxDES::ConvertCiphertext2Hex(char* szPlainInBytes) {
  ...
  int iBitsLen =
      m_iLength % 8 == 0 ? m_iLength << 3 : ((m_iLength >> 3) + 1) << 6;
...
int yxDES::ConvertHex2Ciphertext(const char* szCipherInBytes) {
  ...
  iLen = (((int)strlen(szCipherInBytes) >> 2) +
          ((int)strlen(szCipherInBytes) % 4 == 0 ? 0 : 1))
         << 4;
...
char* yxDES::GetCiphertextData() { return szCiphertextData; }
char* yxDES::GetHexCipherAnyLengthData() { return hexCiphertextAnyLength; }
