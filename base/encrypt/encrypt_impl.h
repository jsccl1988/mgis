...
template <base::EncryptMethod T>
class Dummy_Encrypt : public base::EncryptMethodInterface {
 public:
  ...
  base::EncryptMethod Type() const { return T; }
...
static void ExpandKey(uint32_t ksize, std::string &key) {
  ...
  if (ksize > 16) {
    while (key.size() < ksize) key = base::BinaryToHexString(key);
  }
  if (key.size() > ksize) key.erase(ksize, key.size() - ksize);
}
...
static bool CreateGUID(std::string &guid) {
  ...
  if (::CoCreateGuid(&id) == S_OK) {
    ...
    if (!EVP_DigestUpdate(&mdctx, (const unsigned char *)guid.data(),
                          guid.size())) {
      EVP_MD_CTX_cleanup(&mdctx);
      return false;
    }
...
template <base::EncryptMethod T>
class Openssl_Encrypt_Symmetry_Key : public base::EncryptMethodInterface {
 public:
  Openssl_Encrypt_Symmetry_Key()
      : enable_padding_(false), padding_value_(0), iv_parameter_spec_("") {
    chiper_ = chiper();
  }
  ~Openssl_Encrypt_Symmetry_Key() {}
  ...
  bool Encrypt(const void *sdata, size_t ssize, std::string &ddata) {
    if (!chiper_) return false;
    if (!ddata.empty()) ddata.erase();
    EVP_CIPHER_CTX ctx;
    if (!EVP_EncryptInit(
            &ctx, chiper_, (const unsigned char *)key_.c_str(),
            (iv_parameter_spec_.empty()
                 ? nullptr
                 : (const unsigned char *)iv_parameter_spec_.c_str())))
      return false;
    unsigned char out[256];
    if (enable_padding_) EVP_CIPHER_CTX_set_padding(&ctx, padding_value_);
    ...
    for (uint32_t i = 0; i < block; i++) {
      int lin =
          ((uint32_t)ssize - elen) > osize ? osize : (uint32_t)ssize - elen;
      if (!EVP_EncryptUpdate(&ctx, out, &lout,
                             ((const unsigned char *)sdata) + elen, lin)) {
  ...
  bool Decrypt(const void *sdata, size_t ssize, std::string &ddata) {
    ...
    for (uint32_t i = 0; i < block; i++) {
      int lin =
          (uint32_t)ssize - elen > osize ? osize : ((uint32_t)ssize - elen);
      if (!EVP_DecryptUpdate(&ctx, out, &lout,
                             ((const unsigned char *)sdata) + elen, lin)) {
        EVP_CIPHER_CTX_cleanup(&ctx);
        return false;
      }
  ...
  bool CreateKey(std::string &key1, std::string &key2) {
    ...
    for (;;) {
      ...
      sprintf_s(buf, sizeof(buf), "%X%X", (unsigned int)time(0),
                (unsigned int)rand());
#else
 ...
 private:
  const EVP_CIPHER *chiper() {
    switch (T) {
      ...
      case base::ENC_AES128:
        return EVP_aes_128_ecb();
      case base::ENC_AES192:
        return EVP_aes_192_ecb();
      case base::ENC_AES256:
        return EVP_aes_256_ecb();
      case base::ENC_AES256_CBC:
        return EVP_aes_256_cbc();
      case base::ENC_AES128_CBC:
...
template <base::EncryptMethod T>
class Openssl_Encrypt_Hash : public base::EncryptMethodInterface {
 public:
  ...
  bool Encrypt(const void *sdata, size_t ssize, std::string &ddata) {
    ...
    if (olen > sizeof(obuf)) pbuf = new unsigned char[olen];

    if (!EVP_DigestFinal_ex(&mdctx, pbuf, &olen)) {
      if (pbuf != obuf) delete[] pbuf;
      EVP_MD_CTX_cleanup(&mdctx);
      return false;
    }
...
class IEncryptRSAImpl : public base::EncryptMethodInterface {
 public:
  ...
  //    export methods
  base::EncryptMethod Type() const { return base::ENC_RSA; }
  bool Encrypt(std::string &data) {
    std::string tmp;
  ...
  bool Encrypt(const void *sdata, size_t ssize, std::string &ddata) {
    if (!n_ || !e_ || !sdata || ssize == 0) return false;
    if (!ddata.empty()) ddata.erase();
    ...
    try {
      ...
      for (uint32_t i = 0; i < nBlock; i++) {
        ...
        if ((i + 1) == nBlock && (ssize % block_size != 0) &&
            (RSA_PKCS1_PADDING == padding_mode_ ||
             RSA_PKCS1_OAEP_PADDING == padding_mode_))
          sz = ssize % block_size;
  ...
  bool Decrypt(const void *sdata, size_t ssize, std::string &ddata) {
    if (!n_ || !e_ || !d_ || !sdata || ssize == 0) return false;
    if (!ddata.empty()) ddata.erase();
...
// stream encode
// stream encode have state. When call setEncryptKey()/setDecryptKey() the
// encrypt and decrypt pipe will be reseted. So that we allow user call the
// function setEncryptKey()/setDecryptKey() only once time
template <base::EncryptMethod T>
class Openssl_Encrypt_Stream : public base::EncryptMethodInterface {
 public:
  ...
  ~Openssl_Encrypt_Stream() {
    if (ectx_valid_) EVP_CIPHER_CTX_cleanup(&ectx_);
    if (dctx_valid_) EVP_CIPHER_CTX_cleanup(&dctx_);
  }
  bool SetEncryptKey(const std::string &key) {
    key_ = key;
    ExpandKey(chiper_->key_len, key_);
    ...
    if (!EVP_EncryptInit(
            &ectx_, chiper_, (const unsigned char *)key_.c_str(),
            (iv_parameter_spec_.empty()
                 ? nullptr
                 : (const unsigned char *)iv_parameter_spec_.c_str())))
      return false;
  ...
  bool SetDecryptKey(const std::string &key) {
    ...
    if (!EVP_EncryptInit(
            &dctx_, chiper_, (const unsigned char *)key_.c_str(),
            (iv_parameter_spec_.empty()
                 ? nullptr
                 : (const unsigned char *)iv_parameter_spec_.c_str())))
      return false;
  ...
  size_t Encrypt(const char *ibuf, size_t isize, char *obuf, size_t obsize) {
    ...
    int lout;
    if (!EVP_EncryptUpdate(&ectx_, (unsigned char *)obuf, &lout,
                           (const unsigned char *)ibuf, (int)isize)) {
      assert(false);
      return 0;
    }
  ...
  size_t Decrypt(const char *ibuf, size_t isize, char *obuf, size_t obsize) {
    ...
    if (!chiper_) return 0;
    if (!dctx_valid_) return 0;

    int lout;
    if (!EVP_DecryptUpdate(&dctx_, (unsigned char *)obuf, &lout,
                           (const unsigned char *)ibuf, (int)isize)) {
      assert(false);
      return 0;
    }
  ...
  bool Encrypt(const void *sdata, size_t isize, std::string &ddata) {
    ...
    if (osize > ddata.size()) ddata.resize(osize);
    char *ptr = (char *)&ddata[0];
    size_t sz = Encrypt((const char *)sdata, isize, ptr, osize);
    assert(sz == osize);
    if (ddata.size() > osize) ddata.resize(osize);
    return true;
  }
 ...
 private:
  const EVP_CIPHER *chiper() {
    if (T == base::ENC_ARC4) return EVP_rc4();
    return 0;
  }
...
///////////////////////////////////////////////////////////////////////////////
class BASE_EXPORT Encrypt_Impl : public base::EncryptInterface {
 public:
  Encrypt_Impl() {
    support_methods_[base::ENC_NO] = 1;
    support_methods_[base::ENC_RSA] = 1;
    support_methods_[base::ENC_ARC4] = 1;
    // support_methods_[base::ENC_RC5]  = 1;
    support_methods_[base::ENC_RC2] = 1;
    // support_methods_[base::ENC_IDEA] = 1;
    support_methods_[base::ENC_CAST] = 1;
    support_methods_[base::ENC_AES128] = 1;
    support_methods_[base::ENC_AES128_CBC] = 1;
    support_methods_[base::ENC_AES192] = 1;
    support_methods_[base::ENC_AES256] = 1;
    support_methods_[base::ENC_AES256_CBC] = 1;
    support_methods_[base::ENC_DES64] = 1;
    support_methods_[base::ENC_MD2] = 1;
    support_methods_[base::ENC_MD4] = 1;
    support_methods_[base::ENC_MD5] = 1;
    support_methods_[base::ENC_SHA] = 1;
    support_methods_[base::ENC_SHA1] = 1;
    support_methods_[base::ENC_DSS] = 1;
    support_methods_[base::ENC_DSS1] = 1;
  }
  ...
  base::EncryptMethod Type() const {
    if (!method_) return base::ENC_NO;
    return method_->Type();
  }
  bool Encrypt(std::string &data) {
    if (!method_) return false;
  ...
  //    from iencrypt
  bool SetMethod(base::EncryptMethod type) {
    MethodMap::iterator it = methods_.find(type);
    if (it == methods_.end()) {
      base::EncryptMethodInterface_var e(CreateMethod(type));
      if (!e) return false;
      method_ = e;
      AddMethod(e);
    } else
  ...
  void ListSupportMethods(std::list<base::EncryptMethod> &types) {
    MethodMap::iterator it = methods_.begin();
    for (; it != methods_.end(); it++) types.push_back(it->first);
  }
  bool AddMethod(base::EncryptMethodInterface_var method) override {
    if (!method) return false;
    base::EncryptMethodInterface_var e = method;
  ...
  bool RemoveMethod(base::EncryptMethod type) {
    MethodMap::iterator it = methods_.find(type);
    if (it != methods_.end()) methods_.erase(it);
    return true;
  }
  base::EncryptMethodInterface *CreateMethod(base::EncryptMethod type) {
    if (support_methods_.find(type) == support_methods_.end()) return 0;
    switch (type) {
      case base::ENC_NO:
        return new Dummy_Encrypt<base::ENC_NO>;
      ...
      case base::ENC_ARC4:
        return new Openssl_Encrypt_Stream<base::ENC_ARC4>;
        // case base::RC5:
        //    return new Openssl_Encrypt_Symmetry_Key<base::RC5>;
      case base::ENC_RC2:
        return new Openssl_Encrypt_Symmetry_Key<base::ENC_RC2>;
        // case base::IDEA:
        //    return new Openssl_Encrypt_Symmetry_Key<base::IDEA>;
      case base::ENC_CAST:
        return new Openssl_Encrypt_Symmetry_Key<base::ENC_CAST>;
      case base::ENC_AES128:
        return new Openssl_Encrypt_Symmetry_Key<base::ENC_AES128>;
      case base::ENC_AES128_CBC:
        return new Openssl_Encrypt_Symmetry_Key<base::ENC_AES128_CBC>;
      case base::ENC_AES192:
        return new Openssl_Encrypt_Symmetry_Key<base::ENC_AES192>;
      case base::ENC_AES256:
        return new Openssl_Encrypt_Symmetry_Key<base::ENC_AES256>;
      case base::ENC_AES256_CBC:
        return new Openssl_Encrypt_Symmetry_Key<base::ENC_AES256_CBC>;
      case base::ENC_DES64:
        return new Openssl_Encrypt_Symmetry_Key<base::ENC_DES64>;
      case base::ENC_MD2: {
        assert(0);
        return 0;
        // return new Openssl_Encrypt_Hash<base::ENC_MD2>;
      }
      case base::ENC_MD4:
        return new Openssl_Encrypt_Hash<base::ENC_MD4>;
      case base::ENC_MD5:
        return new Openssl_Encrypt_Hash<base::ENC_MD5>;
      case base::ENC_SHA:
        return new Openssl_Encrypt_Hash<base::ENC_SHA>;
      case base::ENC_SHA1:
        return new Openssl_Encrypt_Hash<base::ENC_SHA1>;
      case base::ENC_DSS:
        return new Openssl_Encrypt_Hash<base::ENC_DSS>;
      case base::ENC_DSS1:
        return new Openssl_Encrypt_Hash<base::ENC_DSS1>;
      default:
        return 0;
    }
  }
 ...
 private:
  typedef std::map<base::EncryptMethod, base::EncryptMethodInterface_var>
      MethodMap;
  MethodMap methods_;
  std::map<base::EncryptMethod, uint32_t> support_methods_;
  base::EncryptMethodInterface_var method_;
};
