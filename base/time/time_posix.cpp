...
Time::TimeStruct Time::ToTimeStruct(bool is_local) {
  ...
  int64_t milliseconds =
      (us_ - kWindowsEpochDeltaMicroseconds) / kMicrosecondsPerMillisecond;
...
Time Time::FromTimeStruct(bool is_local, TimeStruct &ts) {
  ...
  if (seconds == -1 && (ts.year() < 1969 || ts.year() > 1970)) {
    ...
    if (ts.year() < 1969) {
      milliseconds =
          std::numeric_limits<time_t>::min() * kMillisecondsPerSecond;
    } else {
      milliseconds =
          (std::numeric_limits<time_t>::std::max() * kMillisecondsPerSecond) +
...
#elif (defined(OS_POSIX) && defined(_POSIX_MONOTONIC_CLOCK) && \
...
// static
TimeTicks TimeTicks::HighResNow() { return TimeTicks::Now(); }
