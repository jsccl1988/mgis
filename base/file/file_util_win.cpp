...
bool CreateDirectory(const PathChar *full_path) {
  ...
  if (FilePathIsExist(full_path, true)) return true;
...
int WriteFile(const PathChar *filepath, const void *data, size_t size) {
  win32::ScopedWinHandle file(
      CreateFileW(filepath, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, 0, NULL));
  if (!file) return -1;
...
bool DeleteFile(const PathString &filepath) {
  if (::DeleteFileW(filepath.c_str()) != 0) return true;
  return false;
}
