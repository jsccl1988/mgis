...
int ReadFile(const char *filepath, void *data_out, size_t size) {
  ...
  if (int ret = close(fd) < 0) return ret;
  return bytes_read;
}
...
bool CopyFile(const std::string &from_path, const std::string &to_path) {
  ...
  while (result) {
    ...
    do {
      ssize_t bytes_written_partial =
          write(outfile, &buffer[bytes_written_per_read],
  ...
  if (close(infile) < 0) result = false;
  if (close(outfile) < 0) result = false;
...
int64_t GetFileSize(const std::string &filepath) {
  ...
  int test = CallStat(path_str, &file_info);
  if (test != 0) return -1;
  if (!S_ISDIR(file_info.st_mode)) return file_info.st_size;
  return -1;
}
