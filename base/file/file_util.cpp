...
bool IsFilePathSeparator(const PathChar separator) {
  if (separator == kEndChar) return false;

  size_t len = sizeof(kFilePathSeparators) / sizeof(PathChar);
  for (size_t i = 0; i < len; i++) {
    if (separator == kFilePathSeparators[i]) return true;
  }
...
bool FilePathExtension(const PathString &filepath_in,
                       PathString &extension_out) {
  if (filepath_in.size() == 0) return false;
  bool ret = false;
...
bool FilePathApartDirectory(const PathString &filepath_in,
  ...
  if (index <= 0 || filepath_in.size() == 0) return false;
  for (; index != 0; index--) {
...
bool FilePathApartFileName(const PathString &filepath_in,
  ...
  for (size_t index = 0; index < separators_count; index++) {
    ...
    if (separator_pos != PathString::npos) break;
  }
...
bool FilePathCompose(const PathString &directory_in,
                     const PathString &filename_in, PathString &filepath_out) {
  PathString directory;
  if (!FilePathApartDirectory(directory_in, directory)) return false;
...
bool ReadFileToString(const PathString &filepath, std::string &out) {
  std::unique_ptr<FILE, base::DeleterFileClose> file;
#if defined(OS_WIN)
  ...
  if (file_size >
      std::numeric_limits<size_t>::
          max())  // x64编译条件下，size_t的最大值转为int64_t会溢出，因此用无符号
  {
    return false;
  }
