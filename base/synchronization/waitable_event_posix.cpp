...
bool WaitableEvent::Wait() { return WaitTimeout(TimeDelta::FromSeconds(-1)); }
...
static bool cmp_fst_addr(const std::pair<WaitableEvent*, unsigned>& a,
...
// -----------------------------------------------------------------------------
// static
size_t WaitableEvent::EnqueueMultiple(
    std::pair<WaitableEvent*, size_t>* waitables, size_t count,
    Waiter* waiter) {
  if (!count) return 0;
