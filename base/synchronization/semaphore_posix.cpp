...
bool Semaphore::CreateSemaphore(const char *sem_name, uint32_t init_value,
                                uint32_t max_value) {
  if (init_value > max_value) return false;

  init_value_ = init_value;

  /* note : here we will use named semaphore because iOS not supported unamed
   * semaphore*/
  if (sem_name == NULL) {
    sem_unlink("netease_noname_semaphore");
    semaphore_ =
        sem_open("netease_noname_semaphore", O_CREAT | O_EXCL, 0, max_value);
    sem_unlink("netease_noname_semaphore");
    if (semaphore_ == SEM_FAILED) {
      return false;
    }
    for (uint32_t i = 0; i < max_value - init_value; ++i) sem_wait(semaphore_);
  } else {
    sem_unlink(sem_name);
...
bool Semaphore::WaitSemaphore() {
  if (semaphore_) {
    if (sem_wait(semaphore_) == 0) return true;
  }
  return false;
}
