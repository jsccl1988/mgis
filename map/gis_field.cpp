...
int SmtField::GetValueAsDateTime(int *pnYear, int *pnMonth, int *pnDay,
                                 ...
                                 int *pnTZFlag) const {
  if (!IsFieldSetted()) return SMT_ERR_FAILURE;
...
int SmtField::SetValue(const SmtVariant &smtFld) {
  if (m_fldValue.Vt != smtFld.Vt) return SMT_ERR_FAILURE;
  ...
  switch (m_fldValue.Vt) {
    ...
    case SmtString:
      if (NULL == smtFld.bstrVal) return SMT_ERR_FAILURE;

      m_fldValue.bstrVal = strdup(smtFld.bstrVal);
      break;

    case SmtIntegerList: {
      if (NULL == smtFld.iValList.paList) return SMT_ERR_FAILURE;
    ...
    case SmtRealList: {
      if (NULL == smtFld.dbfValList.paList) return SMT_ERR_FAILURE;
    ...
    case SmtStringList: {
      if (NULL == smtFld.bstrValList.paList) return SMT_ERR_FAILURE;
    ...
    case SmtBinary: {
      if (NULL == smtFld.blobVal.paData) return SMT_ERR_FAILURE;
